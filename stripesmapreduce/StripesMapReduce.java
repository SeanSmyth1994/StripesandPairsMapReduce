...
public class StripesMapReduce {
	public static class StripesMapper extends Mapper<LongWritable, Text, Text, MapWritable> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			if (words.length > 1) {
				for (int i = 0; i < words.length; i++) {
					...
					// getting rid of any keys that contain something other than the characters
					// specified.
					if (!newWord.contains(" ")) {
						word.set(newWord);
						stripes.clear();
						...
						for (int j = start; j <= end; j++) {
							...
							String n = nopunct(words[j]);
							Text neighbour = new Text(n);
		...
		// setting a latin based output, should look like:
		// word1 { word2 = 1 .....}
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "Stripes Approach");
		job.setJarByClass(StripesMapReduce.class);
